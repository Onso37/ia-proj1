...
def tactical(player, state):
    ...
    for x in range(1, state.ROWS-1):
        for y in range(1, state.COLS-1):
            if x%2 == x%2:
                if state.board[x][y] == player:
                    score += 1
                elif state.board[x][y] == ((1-player)):
                    score -= 1
    for x in range(0, state.ROWS, state.ROWS-1):
        for y in range(state.COLS):
            if state.board[x][y] == player:
                score -= 0.5
            elif state.board[x][y] == ((1-player)):
                score += 0.5
    for x in range(state.ROWS):
        for y in range(0, state.COLS, state.COLS-1):
            if state.board[x][y] == player:
                score -= 0.5
            elif state.board[x][y] == ((1-player)):
                score += 0.5
...
def in_bounds(position, rows, cols):
...
def dfs(board, player, x, y, count=0):
    ...
    for i in range(4):
        ...
        if in_bounds((new_x, new_y), 0, 0) and board[new_x][new_y] == player:
            count += dfs(board, player, x+directions[i][0], y+directions[i][1], count)

    return count

def bfs(board, sources, player, rows, cols):
    ...
    while len(queue) > 0:
        ...
        for i in range(4):
            new_x = x+directions[i][0]
            new_y = y+directions[i][1]
            if in_bounds((new_x, new_y), rows, cols) and board[new_x][new_y] != 3:
                queue.append(((new_x, new_y), value + 1))

    return score
        
def heuristic3(player, state):
    """Third heuristic. Same as the second, but with special return values for terminal nodes."""
    ...
    if winner == player:
        ...
        return heuristic2(player, state)
    
def heuristic4(player, state):
    """Fourth heuristic. Same as the third, but takes into account specific situations where enemy pieces are surrounded.
    These situations add value to the state."""
    score = 0
    for x in range(state.ROWS):
        for y in range(1 - x%2, state.COLS, 2):
            if state.board[x][y] == player and (not state.can_be_captured((x, y))):
                for nx in [-1,1]:
                    for ny in [-1,1]:
                        if in_bounds((nx, ny), state.ROWS, state.COLS) and state.board[nx][ny] == ((1-player)):
                            score += 1
    ...
    return heuristic3(player, state) + 0.5*score

def heuristic5(player, state):
    """Fifth heuristic. Same as the fourth, but uses a BFS in the lategame, in case the player has the material advantage, to
    reward aggressive behavior."""
    score = 0
    prev = heuristic4(player, state)
    if player == 1:
    ...
    if enemy_pieces < (state.ROWS*state.COLS//2)*0.25 and enemy_pieces <= my_pieces:
        sources = []
        for x in range(state.ROWS):
            for y in range(state.COLS):
                if state.board[x][y] == ((1-player)):
                    sources.append((x, y))
        score = bfs(state.board.copy(), sources, player, state.ROWS, state.COLS)
        prev += my_pieces*min(state.ROWS, state.COLS)
    
    return prev - score


def heuristic6(player, state):
    """Sixth heuristic. Is a weighted combination of what we found to be the most important features in previous heuristics."""
    winner = state.check_win()
    if winner == player:
        return 999999999
